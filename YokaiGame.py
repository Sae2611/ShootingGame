...
img_bg = pygame.image.load("bg_image.png")
...
class Yokai:
    ...
    def reset(self):
        ...
        self.lives = 3
        self.counter = 0
        self.no_kill_counter = 0  # エネミーを撃破できていない時間（60カウント=1秒）
    ...
    def _get_observation(self):
        ...
        for index, alien in enumerate(self.alien_list):
            if alien.alive:
                observation_list.append(alien.rect.center[0]/640)
                observation_list.append(alien.rect.center[1]/480)
        ...
        # ビーム位置情報
        beam_check_range = np.array([128, 128, 256, 32])  # L,R,U,D(学習に使う範囲を指定)
        margin = 120 # ビーム位置用行列の余白
        ...
        for index, shot in enumerate(self.shots):
            observation_list.append(shot.rect.center[0]/640)
            observation_list.append(shot.rect.center[1]/480)
        observation_list.extend([0 for _ in range(8- len(self.shots)*2)])
        '''
        for ufo in self.ufos:
            observation_list.append(ufo.rect.center[0]/640)
            observation_list.append(ufo.rect.center[1]/480)
            observation_list.append(ufo.speed)
        observation_list.extend([0 for _ in range(3 - len(self.ufos) * 3)])

        observation_list.append(self.player.rect.center[0]/640)
        observation_list.append(self.player.rect.center[1]/480)
    ...
    def init_game(self):
        ...
        self.aliens = pygame.sprite.Group()  # エネミーグループ
        ...
        EnemyIcon.containers = self.all, self.aliens, self.invisible
    ...
    def draw(self, screen):
        ...
        if self.game_state == START:  # スタート画面
            ...
            title = title_font.render("妖怪乱舞", False, (0,0,0))
            screen.blit(title, ((SCR_RECT.width-title.get_width())//2, 20))
            # エネミーアイコンを描画
            enemyicon_image = EnemyIcon.images[0]
            screen.blit(enemyicon_image, ((SCR_RECT.width-enemyicon_image.get_width())//2, 60))
            # PUSH STARTを描画
            push_font = pygame.font.Font(FONT_PATH, 20)
            push_space = push_font.render("スペースキーで開始", False, (0,0,0))
            screen.blit(push_space, ((SCR_RECT.width-push_space.get_width())//2, 425))
            ...
            # wave数と残機数を描画
            stat_font = pygame.font.Font(FONT_PATH, 20)
            stat = stat_font.render("レベル:{:2d}  ライフ:{:2d}  得点:{:05d} ".format(
            ...
            # pygame.mixer.music.load("Clearbgm.mp3")
            # pygame.mixer.music.play(-1)


            # 壁の耐久力描画
            shield_font = pygame.font.Font(FONT_PATH, 30)
            ...
            pygame.mixer.music.load("gameoverbgm.mp3")
            pygame.mixer.music.play(0)
            screen.fill((0, 0, 0))
            # GAME OVERを描画
            gameover_font = pygame.font.Font(FONT_PATH, 50)
            gameover = gameover_font.render("任務失敗...", False, (255,0,0))
            ...
            # PUSH SPACEを描画
            push_font = pygame.font.Font(FONT_PATH, 20)
            push_space = push_font.render("スペースキー　で再挑戦", False, (255,255,255))
            screen.blit(push_space, ((SCR_RECT.width-push_space.get_width())//2, 425))
            # pygame.mixer.music.stop()
            # pygame.mixer.music.load("gameoverbgm.mp3")
            # pygame.mixer.music.play(0)
            ...
            stat_font = pygame.font.Font(FONT_PATH, 20)
            stat = stat_font.render("レベル:{:2d}  ライフ:{:2d}  得点:{:05d}".format(
                self.wave, self.lives, self.score), False, (0,0,0))
            screen.blit(stat, ((SCR_RECT.width - stat.get_width()) // 2, 10))
            # STAGE CLEARを描画
            gameover_font = pygame.font.Font(FONT_PATH, 60)
            gameover = gameover_font.render("討伐完了!", False, (0,0,0))
            screen.blit(gameover, ((SCR_RECT.width-gameover.get_width())//2,50))
            # クリアロゴを描画
            playericon_image = Player.images[0]
            screen.blit(playericon_image, ((SCR_RECT.width-playericon_image.get_width())//2, 30))
            # PUSH SPACEを描画
            push_font = pygame.font.Font(FONT_PATH, 20)
            push_space = push_font.render("スペースキーで　次のステージへ", False, (0,0,0))
            screen.blit(push_space, ((SCR_RECT.width-push_space.get_width())//2, 425))
            # pygame.mixer.music.stop()
            pygame.mixer.music.load("Clearbgm.mp3")
            pygame.mixer.music.play(0)
    ...
    def load_images(self):
        """イメージのロード"""
        # スプライトの画像を登録
        Player.image = load_image("player3.png")
        Shot.image = load_image("bom.png")
        Alien.images = split_image(load_image("enemy2.png"), 1)
        EnemyIcon.images = split_image(load_image("gameover.png"),1)
        UFO.images = split_image(load_image("item.png"), 1)
        Beam.image = load_image("fire.png")
        ...
        Player.images = split_image(load_image("ClearIcon.webp"), 1)
    

    def load_sounds(self):
        """サウンドのロード"""
        Alien.kill_sound = load_sound("damage1.mp3")
        Player.shot_sound = load_sound("attack.mp3")
        Player.bomb_sound = load_sound("bomb.wav")
        Wall.guard_sound = load_sound("kill.wav")
        Recover.recover_sound = load_sound("recover.mp3")
...
class EnemyIcon(pygame.sprite.Sprite):
    pass
...
class UFO(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # キャラクターアニメーション
        self.frame += 1
        self.image = self.images[int(self.frame//self.animcycle%1)]
